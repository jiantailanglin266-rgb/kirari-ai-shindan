...
import { saveImage, saveFocus } from "@/lib/store";
...
const FOCUS_OPTIONS: { id: Focus; label: string; emoji: string }[] = [
  { id: "love", label: "恋愛運", emoji: "💗" },
  { id: "money", label: "金運", emoji: "💰" },
  { id: "all", label: "総合運", emoji: "🔮" },
];
...
export function UploadBox() {
  ...
  return (
    <div className="space-y-5">
          ...
          {preview ? (
            ...
            >
              ...
              <button
                ...
                className="absolute right-3 top-3 grid h-9 w-9 place-items-center rounded-full bg-white/10 text-ink shadow-md"
                aria-label="写真を削除"
              >
                <X className="h-4 w-4" />
              </button>
      ...
      {/* 重視する運 */}
      <div>
        <p className="mb-2 text-sm font-bold text-ink">いちばん知りたい運（任意）</p>
      ...
      >
        {starting ? (
          <>
            <Loader2 className="h-5 w-5 animate-spin" /> 鑑定を準備中…
          </>
        ) : (
          "鑑定スタート 🔮"
        )}
