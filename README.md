...
AI顔診断・モテ診断・AI美化加工アプリ（MVP）。
スマホファーストで、TikTok / Instagram / X でシェアされやすい「ポジティブAI診断」エンタメアプリです。

> 単なる顔面偏差値アプリではなく、「AIがあなたの魅力を見つけて、もっと盛れる未来の自分を作る」世界観。
> 低スコアでも必ずポジティブに変換し、ユーザーを傷つけない設計です。
...
| ルート | 内容 |
| --- | --- |
| `/` | LPトップ（ヒーロー・特徴・モテタイプ・料金ティザー・CTA） |
| `/upload` | 写真アップロード（カメラ撮影 / 画像選択 / 規約同意 / タイプ選択） |
| `/analyzing` | AI解析中演出（ステップ表示・スキャンアニメ） |
| `/result` | 診断結果（スコア・レーダー・モテタイプ・恋愛・美容・AI美化・シェア） |
...
SNSシェア（X / Instagram保存 / TikTok保存 / LINE）・今日のモテ運・ガチャ風ランク演出・
